...
- GitHub Pages 版は完全な静的配布です。`data/calibration.json` と研究用確認パネルには
  全160項目の targetword、正答、項目パラメータが含まれるため、項目秘匿が必要な高 stakes
  評価にはそのまま再利用しないでください
- 参加者が保存する結果ファイルには、研究者が再分析・監査できるように反応ログ、正答、
  RT、フォーカス離脱、実施URL、項目情報を含めます。氏名ではなく研究用ID・識別名で運用し、
  倫理審査・同意文書側で保存内容と共有方法を説明してください
...
Adaptive 版は40問フロアなしで停止しますが、報告可否の品質判定ではデフォルトで
Hit / CR 各5回答以上を要求します。これは停止下限ではなく、極端に短い改変URLや
音声失敗・時間切れが多いセッションを `valid_for_reporting=false` として扱うための
監査フラグです。
...
再生成する場合は、親分析環境の `scripts/export_cat_calibration.R` を実行してから、
生成済み `data/calibration.json` を本公開ディレクトリへ同期してください。
...
```text
LJT_result_{id}_{YYYY-MM-DDTHH-MM-SS}.xlsx
```
